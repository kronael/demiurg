...
def _cancel_all(tasks: list[asyncio.Task]) -> None:  # type: ignore[type-arg]
    """signal handler: cancel all async tasks"""
    for t in tasks:
        t.cancel()


def discover_spec(context: tuple[str, ...]) -> list[Path]:
    """resolve spec files from context args or auto-discovery

    with context: single file or all *.md in a directory
    without context: all of SPEC.md, spec.md, specs/*.md
    """
    if context:
        if len(context) == 1:
            p = Path(context[0])
...
def run(
    ...
    try:
        asyncio.run(_main(context, cont, workers, timeout, max_turns, verbosity))
...
async def _main(
    ...
    loop.add_signal_handler(signal.SIGINT, _cancel_all, all_async)
    loop.add_signal_handler(signal.SIGTERM, _cancel_all, all_async)
